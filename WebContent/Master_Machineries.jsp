...
<%@page import="com.webmodel.MachinaryModel"%>
...
<%!ArrayList<MachinaryModel> listMachine;
	...
	public void getList() {
		...
		try {
			...
			String sql = "Select * from master_machineries";
			ResultSet rs = stmt.executeQuery(sql);
			listMachine = new ArrayList<MachinaryModel>();
			while (rs.next()) {
				MachinaryModel item = new MachinaryModel();
				item.setId(rs.getString("Machineries_Id"));
				item.setIsActive(rs.getString("Is_Active"));
				item.setMachinName(rs.getString("Machineries"));
...
<body>
	...
	<div class="container">
		<table class="col-lg-12">
			<tr>
				<td><a href="AddNew.jsp?id=<%=PasswordEncryption.encryptBase64("" + WebConstants.Machinary)%>">
		...
		<div class="details">
			<table id="example" class="display table table-bordered">
						<thead>
							...
							<tr>
							<td><%=i+1%></td>
								<td><%=listMachine.get(i).getMachinName()%></td>
								<td><span id="ContentPlaceHolder1_GVManpower_lblStatus_0"><%=listMachine.get(i).getIsActive()%></span>
								</td>
								<td><a
									href="EditFile.jsp?id=<%=PasswordEncryption.encryptBase64(listMachine.get(i).getId())%>&Type=<%=PasswordEncryption.encryptBase64("" + WebConstants.Machinary)%>">Edit</a></td>
							</tr>
