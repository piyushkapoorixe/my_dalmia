...
<%@page import="com.webmodel.MasterQualification"%>
...
ArrayList<MasterQualification> listMachine;
public void getList() {
	Connection con=null;
		try {
			 con = DBConnectionProvider.getCon();
			Statement stmt = con.createStatement();
			String sql = "Select * from master_qualification";
			ResultSet rs = stmt.executeQuery(sql);
			listMachine = new ArrayList<MasterQualification>();
			while (rs.next()) {
				MasterQualification item = new MasterQualification();
				item.setId(rs.getString("Qualification_Id"));
...
<body>
	...
	<div class="container">
		<table class="col-lg-12">
			<tr>
				<td><a
					href="AddNew.jsp?id=<%=PasswordEncryption.encryptBase64("" + WebConstants.Qualification)%>">
		...
		<div class="details">
			<table id="example" class="display table table-bordered">
				<thead>
					...
					<tr>
						<td><%=i + 1%></td>
						<td><%=listMachine.get(i).getQualification()%></td>
						<td><span id="ContentPlaceHolder1_GVManpower_lblStatus_0"><%=listMachine.get(i).getIsActive()%></span>
						</td>
						<td><a
							href="EditFile.jsp?id=<%=PasswordEncryption.encryptBase64(listMachine.get(i).getId())%>&Type=<%=PasswordEncryption.encryptBase64("" + WebConstants.Qualification)%>">Edit</a></td>
					</tr>
				<%
					}
					}
