...
ArrayList<ManPowerModel> listMachine;
public void getList() {
	Connection con=null;
		try {
			con = DBConnectionProvider.getCon();
			Statement stmt = con.createStatement();
			String sql = "Select * from master_manpower";
			ResultSet rs = stmt.executeQuery(sql);
			listMachine = new ArrayList<ManPowerModel>();
			while (rs.next()) {
				ManPowerModel item = new ManPowerModel();
				item.setId(rs.getString("ManpowerId"));
				item.setIsActive(rs.getString("Is_Active"));
				item.setManpowerName(rs.getString("Manpower"));
...
<body>
	...
	<div class="container">
		<table class="col-lg-12">
			<tr>
				<td><a
					href="AddNew.jsp?id=<%=PasswordEncryption.encryptBase64("" + WebConstants.ManPower)%>">
		...
		<div class="details">
			<table id="example" class="display table table-bordered">
				...
				<tr>
					<td><%=i + 1%></td>
					<td><%=listMachine.get(i).getManpowerName()%></td>
					<td><span id="ContentPlaceHolder1_GVManpower_lblStatus_0"><%=listMachine.get(i).getIsActive()%></span>
					</td>
					<td><a
						href="EditFile.jsp?id=<%=PasswordEncryption.encryptBase64(listMachine.get(i).getId())%>&Type=<%=PasswordEncryption.encryptBase64("" + WebConstants.ManPower)%>">Edit</a></td>
				</tr>
				<%
					}
					}
